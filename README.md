...
The notebook clones this repository over HTTPS at tag `v0.1.16`; Kaggle needs no SSH key. A Kaggle Dataset containing the required GGUF folder can be attached to save GPU-time downloads. The fallback downloads the official modules from Hugging Face. During the human experiment, Granite 350M and Whisper Tiny run in FP16 on GPU 1 while the native MiniCPM runtime can use both GPUs.

The human UI submits each turn as a background job and polls it with short requests. This keeps a long MiniCPM turn independent of the public Gradio queue connection. Its latest stage and result are also written to `duplex_voice_output/latest_turn.json`. Granite routing has a 90-second deadline so a stalled CPU caller is recorded as an experimental failure.
